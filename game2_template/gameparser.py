...
def filter_words(words, skip_words):
    ...
    """
    filteredlist = [x for x in words if x not in skip_words]
    return filteredlist
...
def normalise_input(user_input):
    ...
    clearedinput = remove_punct(user_input).strip().lower().split()
    concludedinput = filter_words(clearedinput, skip_words)
    return concludedinput
    #
    # COMPLETE ME!
    #
