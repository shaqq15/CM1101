...
def normalise_input(user_input):
    ...
    'help'
    """
    nospace= remove_spaces(user_input)
    nopunctuation= remove_punct(nospace)
    return nopunctuation.strip().lower()
...
def print_menu(exits):
   ...
   for l in exits:
       leads_to = exit_leads_to(exits,location)
       print_menu_line(location, leads_to)
...
def menu(exits):
    ...
    while True:
        ...
        # Normalise the input
        normalisedinput = normalise_input(playersinput)

        # Check if the input makes sense (is valid exit)
            # If so, return the player's choice
        if is_valid_exit(exits, normalisedinput) == True:
